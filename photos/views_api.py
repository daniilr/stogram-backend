...
class Feed(APIView):
    ...
    def get(self, request):
        ...
        data = []
        for post in Post.objects.all().order_by("-id")[offset:offset+limit]:
            post_serialized = FeedSerializer(post).data
            post_serialized['is_liked'] = True if post.like_set.filter(user=request.user).count() else False
            data.append(post_serialized)
        return Response({
            "feed": data
        })
...
class PostLike(APIView):
    ...
    def get(self, request, post_id):
        try:
            ...
            raise exceptions.NotFound("Post was not found.")
        return Response({
            "is_liked": not post.toggle_like(request.user),
...
class CommentView(APIView):
    ...
    def post(self, request, post_id):
        ...
        return Response({
            "comments": [CommentSerializer(comment).data for comment in post.comment_set.all().reverse()[:20]]
        })
...
class TemplateList(APIView):
    ...
    def get(self, request):
        return Response({
            "templates": [TemplatesSerializer(template).data for template in Template.objects.all()]
        })
